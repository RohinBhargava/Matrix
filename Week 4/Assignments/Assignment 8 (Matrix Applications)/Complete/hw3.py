...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return Vec(M.D[1], { i : mat2coldict(M)[i] * v for i in M.D[1] })
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return coldict2mat({ i : A * mat2coldict(B)[i] for i in B.D[1]})
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return rowdict2mat({ i : mat2rowdict(A)[i] * B for i in A.D[0]})
...
def dot_prod_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return Mat((A.D[0],B.D[1]), {(i,j) : mat2rowdict(A)[i] * mat2coldict(B)[j] for i in mat2rowdict(A).keys() for j in mat2coldict(B).keys()})
