...
indep_vec_1 = Vec({1}, {1 : 1})
indep_vec_2 = Vec({1}, {1 : 2})
indep_vec_3 = Vec({1}, {1 : 3})
indep_vec_4 = Vec({1}, {1 : -6})
...
'''Rohin's note: There are multiple answers, but the correct answers have to be able to be replaced by the lemma without changing the set's span.'''
...
'''Rohin's note: See above problem note.'''
...
def superset_basis(S, L):
    ...
    Example:
        ...
        >>> a2 = Vec({'a','b','c','d'}, {'c':1})
        >>> a3 = Vec({'a','b','c','d'}, {'a':1,'c':3})
        >>> superset_basis([a0, a3], [a0, a1, a2]) == [Vec({'a', 'c', 'b', 'd'},{'a' : 1, 'c': 3}), Vec({'a', 'c', 'b', 'd'},{'a': 1}), Vec({'a', 'c', 'b', 'd'},{'b':1})]
        True
    '''
    T = S.copy()
    for y in S:
    ...
    counter = 0
    for x in returnlist:
        if x not in S and is_superfluous(returnlist, counter):
            returnlist.remove(x)
        counter += 1
    return returnlist
...
def exchange(S, A, z):
    ...
    counter = 0
    for j in union:
        if is_superfluous(union, counter) and j not in A and j != z:
            return j
        counter += 1
