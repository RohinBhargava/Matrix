...
def morph(S, B):
    ...
    T = list()
    for x in B:
        T.append((x, exchange(S, B, x)))
    return T

## Problem 4
# Please express each solution as a list of vectors (Vec instances)

'''Rohin's note: There are multiple answers, this is just one of them. In determining a basis you can use any combination of vectors within the span of the set, provided that they are linearly independent.'''

row_space_1 = [Vec({0, 1, 2}, { 0 : 1, 1 : 2, 2 : 0 }), Vec({0, 1, 2}, { 0 : 0, 1 : 2, 2 : 1 })]
col_space_1 = [Vec({0, 1}, { 0 : 1 , 1 : 0 }), Vec({0, 1}, { 0 : 2, 1 : 2 })]

row_space_2 = [Vec({0, 1, 2, 3}, { 0 : 1, 1 : 4, 2 : 0, 3 : 0}), Vec({0, 1, 2, 3}, { 0 : 1, 0 : 2, 2 : 2, 3 : 0}), Vec({0, 1, 2, 3}, { 0 : 0, 1 : 0, 2 : 1, 3 : 1})]
...
def my_is_independent(L): 
    ...
    False
    '''
    return False if rank(L) < len(L) else True
...
def subset_basis(T): 
    ...
    Rohin's note: This would be the case where the last vector in the set is removed. There are other alternatives, as long as the definition of the basis is upheld.
    '''
    while not my_is_independent(T):
        T.remove(T[0])
    return T
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    True
    '''
    return (rep2vec(vec2rep(U_basis, w), U_basis), rep2vec(vec2rep(V_basis, w), V_basis))
...
def find_matrix_inverse(A):
    ...
    while counter < len(A.D[0]):
        resultantVec.f[list(A.D[0])[counter]] = one
        if counter > 0:
            resultantVec.f[list(A.D[0])[counter - 1]] = 0
        matrix.append(solve(A,resultantVec))
        counter += 1
